...
let messages ~config ~body ~extra_headers ~stream =
  ...
  | None ->
    ...
    let%lwt resp, resp_body =
      Ai_provider.Http_client.post ~timeouts:config.timeouts ~provider:"anthropic" ~headers:cohttp_headers
    ...
    | true, _ ->
      ...
      let retry_after_ms = Cohttp.Header.get resp_headers "retry-after-ms" in
      let retry_after = Cohttp.Header.get resp_headers "retry-after" in
      let err = Anthropic_error.of_response ~retry_after_ms ~retry_after ~status ~body:body_str () in
      Lwt.fail (Ai_provider.Provider_error.Provider_error err)
    | false, true ->
      Lwt.return
        (`Stream
           (Ai_provider.Http_client.wrap_body_with_idle_timeout ~timeouts:config.timeouts ~provider:"anthropic"
              resp_body))
    | false, false ->
      let%lwt body_str = Cohttp_lwt.Body.to_string resp_body in
