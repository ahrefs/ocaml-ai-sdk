...
let print_metrics label ~input ~output pm =
  ...
  let cache_read =
    match md with
    | Some m -> string_of_int m.cache_read_tokens
    | None -> "-"
  in
  let cache_write =
    match md with
    | Some m -> string_of_int m.cache_write_tokens
    | None -> "-"
  in
...
let top_level_provider_options =
  let opts = { OR.Openrouter_options.default with cache_control = Some { type_ = "ephemeral"; ttl = Some "5m" } } in
...
let run_top_level label prompt =
  ...
  let%lwt r =
    Ai_core.Generate_text.generate_text ~model ~system:big_system_prompt ~provider_options:top_level_provider_options
      ~prompt ()
  in
