...
let test_finish_reason_round_trip () =
  ...
  List.iter
    (fun r ->
      ...
      Alcotest.(check string) "round trip" (to_string r) (to_string r'))
    cases

let test_finish_reason_other () =
  let r = Ai_provider.Finish_reason.of_string "something_new" in
  match r with
  | Ai_provider.Finish_reason.Other s -> Alcotest.(check string) "captures unknown" "something_new" s
...
let test_usage_construction () =
  ...
  Alcotest.(check (option int)) "total" (Some 150) u.total_tokens

let test_usage_no_total () =
  let u : Ai_provider.Usage.t = { input_tokens = 100; output_tokens = 50; total_tokens = None } in
  Alcotest.(check (option int)) "no total" None u.total_tokens
...
let test_provider_error_api () =
  ...
  Alcotest.(check bool) "contains status" true (String.length s > 0)

let test_provider_error_exception () =
  let e : Ai_provider.Provider_error.t = { provider = "test"; kind = Network_error { message = "timeout" } } in
  Alcotest.check_raises "raises Provider_error" (Ai_provider.Provider_error.Provider_error e) (fun () ->
    raise (Ai_provider.Provider_error.Provider_error e))

let () =
  Alcotest.run "Foundation_types"
    [
      ( "finish_reason",
        [
          Alcotest.test_case "round_trip" `Quick test_finish_reason_round_trip;
          Alcotest.test_case "other" `Quick test_finish_reason_other;
        ] );
      ( "usage",
        [
          Alcotest.test_case "construction" `Quick test_usage_construction;
          Alcotest.test_case "no_total" `Quick test_usage_no_total;
        ] );
      ( "warning",
        [
          Alcotest.test_case "unsupported" `Quick test_warning_unsupported;
          Alcotest.test_case "other" `Quick test_warning_other;
        ] );
      ( "provider_error",
        [
          Alcotest.test_case "api_error" `Quick test_provider_error_api;
          Alcotest.test_case "exception" `Quick test_provider_error_exception;
        ] );
    ]
