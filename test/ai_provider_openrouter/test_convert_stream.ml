...
let test_encrypted_reasoning_stream () =
  ...
  let parts = collect_stream stream in
  let reasoning_parts =
    List.filter (function Ai_provider.Stream_part.Reasoning _ -> true | _ -> false) parts
  in
  (check int) "one reasoning part" 1 (List.length reasoning_parts);
  (match reasoning_parts with
  | [ Reasoning { text } ] -> (check string) "redacted" "[REDACTED]" text
  | _ -> fail "expected Reasoning [REDACTED]")
...
let test_error_chunk_stream () =
  let events =
    Lwt_stream.of_list
      [ make_sse_event {|{"error":{"message":"Rate limit exceeded","code":429}}|} ]
  in
  let stream = Ai_provider_openrouter.Convert_stream.transform events ~warnings:[] in
  let parts = collect_stream stream in
  let error_parts =
    List.filter (function Ai_provider.Stream_part.Error _ -> true | _ -> false) parts
  in
  (check int) "one error" 1 (List.length error_parts);
  let finish_parts =
    List.filter (function Ai_provider.Stream_part.Finish _ -> true | _ -> false) parts
  in
  (check int) "one finish" 1 (List.length finish_parts);
...
let test_finish_reason_override_encrypted_tool_calls_stream () =
  ...
  (match finish_parts with
  | [ Finish { finish_reason; _ } ] ->
    (check string) "overridden to tool_calls" "tool-calls"
    ...
    [
      ( "convert_stream",
        [
          ...
          test_case "finish_override_encrypted_tools" `Quick
            test_finish_reason_override_encrypted_tool_calls_stream;
